...
function hugeit_share_show_hide_shares_count() {
	...
	$shares_counts = $wpdb->get_results($query, ARRAY_N)[0][0];
...
function hugeit_share_options_panel() {
	...
	add_submenu_page('huge_it_share_buttons', 'Featured Plugins', 'Featured Plugins', 'manage_options', 'huge_it_share_buttons_featured_plugins', 'hugeit_share_featured_plugins' );
...
function hugeit_share_buttons_action_callback() {
	...
	if ( $_POST["post_name"] == "sharebuttonspostsidebar" ) {
		...
		$postactive = sanitize_text_field($_POST["active"]);
